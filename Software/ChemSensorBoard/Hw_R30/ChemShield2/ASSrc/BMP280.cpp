...
char BMP280::readTemperatureRegisters(long &adcTemperature) {
  ...
  bool result = I2CB.read(BMP280ADDRESS, BMP280_REG_TEMP_MSB, 0x01, data, 0x03);
  if (result) {
...
char BMP280::readPressureRegisters(long &adcPressure) {
  unsigned char data[3];
  bool result = I2CB.read(BMP280ADDRESS, BMP280_REG_PRESS_MSB, 0x01, data, 0x03);
  if (result) {
...
char BMP280::readData(char address, unsigned char &value) {
	...
	bool result = I2CB.read(BMP280ADDRESS, address, 0x01, &value, 0x01);
	return (result)? 1 : 0;
}
...
char BMP280::readCalibRegister(char address, short *value) {
	...
	bool result = I2CB.read(BMP280ADDRESS, address, 0x01, data, 0x02);

	*value = 0;
	if (result) {
		*value =  ((data[1] << 8) & 0xFF00) | data[0];
	}
	return (result)? 1 : 0;
}
...
char BMP280::writeRegister(unsigned char address, unsigned char value) {
	...
	bool result = I2CB.write(BMP280ADDRESS, address, 0x01, &value, 0x01);
	return (result)? 1:0;
}
