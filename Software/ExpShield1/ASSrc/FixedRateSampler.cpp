...
FixedRateSampler::FixedRateSampler(unsigned char channels, SensorDevice *sensorDevice, unsigned char fixedPrescaler)
																: Sampler(channels, sensorDevice), fixedDecimation(false) {
	Sampler::setPreScaler(fixedPrescaler);
}

// Prescaler and decimation are set at construction time
FixedRateSampler::FixedRateSampler(unsigned char channels, SensorDevice *sensorDevice, unsigned char fixedPrescaler, unsigned char fixedDecimation)
																: Sampler(channels, sensorDevice), fixedDecimation(true) {
	Sampler::setPreScaler(fixedPrescaler);
